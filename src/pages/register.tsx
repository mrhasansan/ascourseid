  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 p-5">
      <main className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 pb-16 md:grid-cols-2">
        <section className="relative hidden overflow-hidden rounded-2xl bg-blue-600 md:block">
        ...
        <section>
          <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/[0.06] md:p-8">
            <h2 className="mb-4 text-center text-2xl font-bold text-blue-600">As Course</h2>
              ...
              {[1, 2, 3].map((n) => (
                <div key={n} className="flex items-center gap-3">
                  <button
                    ...
                    className={`grid h-8 w-8 place-items-center rounded-full border text-sm transition ${activeStep === n ? "bg-blue-600 text-white border-blue-600" : "border-neutral-300 text-neutral-500 hover:bg-neutral-100"}`}
                  >
                    {n}
                  </button>
                  {n !== 3 && (
            ...
            <div className="mt-6 grid grid-cols-2 overflow-hidden rounded-xl border border-neutral-200 p-1">
              <button type="button" onClick={() => setRole("tentor")} className={`rounded-lg px-3 py-2 text-sm font-medium transition ${role === "tentor" ? "bg-blue-600 text-white" : "text-neutral-700 hover:bg-neutral-100"}`}>
                Daftar sebagai Tentor
              </button>
              <button type="button" onClick={() => setRole("siswa")} className={`rounded-lg px-3 py-2 text-sm font-medium transition ${role === "siswa" ? "bg-blue-600 text-white" : "text-neutral-700 hover:bg-neutral-100"}`}>
            ...
            <div className="mt-10 flex justify-end">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-blue-600/40"
              >
                Isi via Google Form
              </a>
            </div>
